SELECT
    ...
    , (SELECT r1."cdref_mei" FROM M05_CD r1 WHERE r1."cdref_cd" = a."cdref_cdcdref_cd") AS "cdref_mei"
    , TRIM(TRAILING ' ' FROM a."noref_no") AS "noref_no"
    , (SELECT r2."noref_mei" FROM M05_NO r2 WHERE r2."noref_no" = a."noref_nonoref_no") AS "noref_mei"
    , a."ex_idref_id" AS "ex_idref_id"
    , (SELECT r3."idref_mei" FROM M05_ID r3 WHERE r3."idref_id" = a."ex_idref_ididref_id") AS "ex_idref_mei"
    , a."ex_idbn_bn" AS "ex_idbn_bn"
    , (SELECT r4."idbn_no" FROM M05_IDBN r4 WHERE r4."idref_id" = a."ex_idbn_bnidref_id" AND r4."idbn_bn" = a."ex_idbn_bnidbn_bn") AS "ex_idbn_no"
    , a."oya_saiki_id" AS "oya_saiki_id"
    , (SELECT r5."saiki_mei" FROM M05_SAIKI r5 WHERE r5."saiki_id" = a."oya_saiki_idsaiki_id") AS "oya_saiki_mei"
    , TO_CHAR (a."insert_ts", 'YYYY-MM-DD HH24:MI:SS.MS') AS "insert_ts"
    , TRIM(TRAILING ' ' FROM a."insert_user_id") AS "insert_user_id"
    , (SELECT r6."user_sei" FROM MHR_USER r6 WHERE r6."user_id" = a."insert_user_iduser_id") AS "insert_user_sei"
    , TO_CHAR (a."update_ts", 'YYYY-MM-DD HH24:MI:SS.MS') AS "update_ts"
    , TRIM(TRAILING ' ' FROM a."update_user_id") AS "update_user_id"
    , (SELECT r7."user_sei" FROM MHR_USER r7 WHERE r7."user_id" = a."update_user_iduser_id") AS "update_user_sei"
FROM
    M05_SAIKI a 
WHERE
    1 = 1 
