SELECT
    ...
    , (SELECT r4.`IDBN_NO` FROM M04_IDBN r4 WHERE r4.`IDREF_ID` = a.`EX_IDREF_ID` AND r4.`IDBN_BN` = a.`EX_IDBN_BN`) AS `EX_IDBN_NO`
    , a.`OYA_SAIKI_ID` AS `OYA_SAIKI_ID`
    , (SELECT r5.`SAIKI_MEI` FROM M04_SAIKI r5 WHERE r5.`SAIKI_ID` = a.`OYA_SAIKI_ID`) AS `OYA_SAIKI_MEI`
