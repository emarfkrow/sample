...
public class Tb4KoOrphansSRegistForm implements IForm {
    ...
    public void validate(final Map<String, String> errors, final BaseProcess baseProcess) {
        for (int i = 0; i < this.tb4KoOrphansGrid.size(); i++) {
            Tb4KoOrphansRegistForm form = this.tb4KoOrphansGrid.get(i);
            if (form != null) {
                Map<String, String> gridErrors = new java.util.LinkedHashMap<String, String>();
                form.validate(gridErrors, baseProcess);
                BaseProcess.copyGridErrors(errors, "Tb4KoOrphansGrid", i, gridErrors);
            }
        }
    }
